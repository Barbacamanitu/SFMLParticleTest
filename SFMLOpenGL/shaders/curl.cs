...
vec2 calculateAcceleration(vec2 particlePosition)
{
	...
	vec2 field = vec2(cos(particlePosition.x+1600*particlePosition.y), sin(particlePosition.x-800*particlePosition.y));
	accelDirection += (normalize(field) * -.1f);
  ...
  void main()
{
	...
	//vec2 field = vec2(cos(currentPosition.x+1600*currentPosition.y), sin(currentPosition.x-800*currentPosition.y));
	//newVelocity += field;
